...
# 1. Параметри варіанта 4 (N = 4, k = 0.5)
N = 4
k = 0.5

S_A = 0.5 * N * k      # 1.0 од. сировини на 1 стіл А
T_A = N / k            # 8.0 од. часу на 1 стіл А
S_B = float(N)         # 4.0 од. сировини на 1 стіл В
T_B = 0.4 * N * k      # 0.8 од. часу на 1 стіл В
W_A = float(N * k)     # 2.0 грн прибутку від 1 столу А
W_B = 1.2 * N * k      # 2.4 грн прибутку від 1 столу В
...
# ОДР обмежена:
# 1) x1 >= 0, x2 >= 0
# 2) 1.0*x1 + 4.0*x2 <= 1700  => при x1=0: x2=425; при x2=0: x1=1700
# 3) 8.0*x1 + 0.8*x2 <= 1600  => при x1=0: x2=2000; при x2=0: x1=200

# Точка перетину прямих обмежень L1 та L2:
# 1.0*x1 + 4.0*x2 = 1700
# 8.0*x1 + 0.8*x2 = 1600
...
vertices = {
    'O': (0.0, 0.0),
    'A': (LIMIT_TIME / T_A, 0.0),               # (200.0, 0.0)
    'B': (x1_opt, x2_opt),                      # (161.54, 384.62)
...
int_x1, int_x2 = int(round(res_int.x[0])), int(round(res_int.x[1]))
...
# Вимога: без жирного шрифту (fontweight='normal') та без довгих тире
plt.style.use('seaborn-v0_8-whitegrid' if 'seaborn-v0_8-whitegrid' in plt.style.available else 'default')
fig, ax = plt.subplots(figsize=(10, 7.5), dpi=300)

x1_vals = np.linspace(0, 300, 1000)

# Прямі обмежень
# 1) 1.0*x1 + 4.0*x2 = 1700 => x2 = 425 - 0.25*x1
L1 = (LIMIT_RAW - S_A * x1_vals) / S_B
# 2) 8.0*x1 + 0.8*x2 = 1600 => x2 = 2000 - 10*x1
L2 = (LIMIT_TIME - T_A * x1_vals) / T_B

ax.plot(x1_vals, L1, label=r'$L_1:\ 1.0x_1 + 4.0x_2 = 1700$ (Сировина)', color='#1f77b4', linewidth=2)
ax.plot(x1_vals, L2, label=r'$L_2:\ 8.0x_1 + 0.8x_2 = 1600$ (Машинний час)', color='#d62728', linewidth=2)

# Межі ОДР
poly_x = [0, 200, x1_opt, 0]
...
# Вектор градієнта grad W = (2.0, 2.4)
grad_scale = 50.0
gx, gy = W_A * grad_scale, W_B * grad_scale
ax.quiver(0, 0, gx, gy, angles='xy', scale_units='xy', scale=1, 
          color='#800080', width=0.007, label=r'Вектор градієнта $\vec{c} = (2.0;\ 2.4)$', zorder=6)
ax.annotate(r'$\vec{c} = \nabla W = (2.0; 2.4)$', xy=(gx, gy), xytext=(12, -8),
            textcoords='offset points', fontsize=10, fontweight='normal', color='#800080')

# Лінії рівня цільової функції W = 2.0*x1 + 2.4*x2 = const
w_levels = [400, 800, 1020, w_max]
for idx, wl in enumerate(w_levels):
    line_level = (wl - W_A * x1_vals) / W_B
    is_opt = (idx == len(w_levels) - 1)
    ls = '-' if is_opt else '--'
    lw = 2.2 if is_opt else 1.2
    c_color = '#e65100' if is_opt else '#ff9800'
    label = f'Оптимальна лінія рівня $W = {wl:.1f}$' if is_opt else (f'Лінії рівня $W = const$' if idx == 0 else None)
...
ax.scatter([x1_opt], [x2_opt], color='#d9534f', s=140, edgecolor='black', linewidth=1.5, zorder=7, 
           label=f'Точка максимуму B({x1_opt:.2f}; {x2_opt:.2f})')

ax.set_xlim(-15, 270)
ax.set_ylim(-20, 500)
ax.set_xlabel(r'Кількість столів моделі А ($x_1$), шт.', fontsize=11, fontweight='normal')
ax.set_ylabel(r'Кількість столів моделі В ($x_2$), шт.', fontsize=11, fontweight='normal')
ax.set_title('Графічне розв’язання задачі лінійного програмування (Варіант 4)\n' + 
             r'$W = 2.0x_1 + 2.4x_2 \to \max,\quad 1.0x_1 + 4.0x_2 \leq 1700,\quad 8.0x_1 + 0.8x_2 \leq 1600$', 
...
z_x = np.linspace(150, 175, 500)
z_L1 = (LIMIT_RAW - S_A * z_x) / S_B
z_L2 = (LIMIT_TIME - T_A * z_x) / T_B

ax_z.plot(z_x, z_L1, label=r'Межа $L_1$: $1.0x_1 + 4.0x_2 = 1700$ (сировина)', color='#1f77b4', linewidth=2.2)
ax_z.plot(z_x, z_L2, label=r'Межа $L_2$: $8.0x_1 + 0.8x_2 = 1600$ (час)', color='#d62728', linewidth=2.2)

# Заливка ОДР у зумі
z_fill_y = np.minimum(z_L1, z_L2)
ax_z.fill_between(z_x, 370, z_fill_y, color='#2ca02c', alpha=0.2, label='Допустима область')

# Сітка цілих точок в околі
int_x_pts = np.arange(153, 172)
int_y_pts = np.arange(376, 392)
for ix in int_x_pts:
    for iy in int_y_pts:
        is_feas = (S_A * ix + S_B * iy <= LIMIT_RAW) and (T_A * ix + T_B * iy <= LIMIT_TIME)
        if is_feas:
            if ix == int_x1 and iy == int_x2:
                ax_z.scatter(ix, iy, color='#ff9900', s=130, marker='*', edgecolor='black', zorder=6,
...
# Лінія рівня W = 1246.15
z_opt_line = (w_max - W_A * z_x) / W_B
ax_z.plot(z_x, z_opt_line, color='#e65100', linestyle='--', linewidth=1.8, label=f'Лінія рівня W = {w_max:.2f}')

# Лінія рівня для цілочисельного W = 1244.0
z_int_line = (w_int - W_A * z_x) / W_B
ax_z.plot(z_x, z_int_line, color='#ff9900', linestyle=':', linewidth=1.8, label=f'Лінія рівня W = {w_int:.1f}')

ax_z.set_xlim(154, 170)
ax_z.set_ylim(376, 390)
ax_z.set_xlabel(r'Кількість столів моделі А ($x_1$), шт.', fontsize=11, fontweight='normal')
ax_z.set_ylabel(r'Кількість столів моделі В ($x_2$), шт.', fontsize=11, fontweight='normal')
ax_z.set_title('Деталізований окіл точки оптимуму та цілочисельний аналіз\n' + 
               f'Неперервний розв’язок: ({x1_opt:.2f}; {x2_opt:.2f}) | Цілочисельний розв’язок: ({int_x1}; {int_x2})',
               fontsize=11.5, fontweight='normal', pad=12)
...
# Стилізація таблиці (без жирного шрифту)
